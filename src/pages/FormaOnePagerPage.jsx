...
export default function FormaOnePagerPage() {
  const [open, setOpen] = useState(false)

  return (
    <div style={{ position: 'relative', width: '100%', height: '100vh' }}>
      <iframe
        src="/forma-one-pager.html"
        className="w-full h-screen border-0"
        title="viax Forma One Pager"
      />
        ...
        {open && (
          <div style={{
            ...
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
              <Btn onClick={() => downloadFile('/forma-one-pager.html', 'viax-forma-one-pager.html')}>
                <IconDownload /> HTML
              </Btn>
              <Btn onClick={() => downloadFile('/viax-Forma.pdf', 'viax-Forma.pdf')}>
                <IconDownload /> PDF
              </Btn>
              <Btn onClick={() => window.open('/forma-one-pager.html', '_blank')}>
                <IconExternal /> View
              </Btn>
            </div>
          </div>
